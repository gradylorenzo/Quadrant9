...
public class StatusRingController : MonoBehaviour {
    ...
    private void UpdateMeters()
    {
        ...
        //
        speedFill = (GameManager._playerShip.GetComponent<ShipTransform>().CurrentThrottle) * .22f;
        SpeedBar.fillAmount = speedFill;
        if (speedFill > 0)
        {
            SpeedRead.text = ((GameManager._playerShip.GetComponent<ShipTransform>().CurrentThrottle * GameManager._playerShip.GetComponent<ShipController>().currentAttributes._travel._burnSpeed) * 1000).ToString("###") + " m/s";
        }
        else
        {
